...
# DEBUG_KERNEL_DIR = "./debug_kernels"
# os.makedirs(DEBUG_KERNEL_DIR, exist_ok=True)

def generate_gaussian_batch_checkpointed(coords, colours, scale_all, rotation_all, opacity, xy_base, device, start_idx, end_idx, image_width, image_height, kernel_size):
    ...
    # print("coords batch:",coords_batch)

    coords_px = ((coords_batch + 1) / 2) * (kernel_size - 1)
    # print("coords px:",coords_px)

    kernel_center = kernel_size // 2
    crop_tops = (kernel_center - coords_px[:, 1] // 2).long()
    crop_lefts = (kernel_center - coords_px[:, 0] //2 ).long()

    # print(crop_tops,crop_lefts)
    
    crop_tops = crop_tops.clamp(0, kernel_size - image_height)
    crop_lefts = crop_lefts.clamp(0, kernel_size - image_width)

    # print(crop_tops, crop_lefts)

    # print(f"Crop tops: {crop_tops.shape}")
    # print(f"Crop lefts: {crop_lefts.shape}")
    
    cropped_kernels = []
    for i in range(current_batch):
        cropped = kernel_rgb[i, :, crop_tops[i]:crop_tops[i]+image_height,
                               crop_lefts[i]:crop_lefts[i]+image_width]
        cropped_kernels.append(cropped)
        
    kernel_rgb_cropped = torch.stack(cropped_kernels, dim=0)

    # print(f"\nFinal cropped kernels tensor: {kernel_rgb_cropped.shape}")

    # for i in range(min(3, kernel_rgb_cropped.shape[0])):  # only first few per batch
    #     save_debug_kernel(kernel_rgb_cropped[i], i)

    # Translate the kernel
    # b, c, h, w = kernel_rgb_cropped.shape
    ...
    # kernel_rgb_padded_translated = F.grid_sample(kernel_rgb_cropped, grid, mode='bilinear',padding_mode='zeros',align_corners=False)

    # Apply colors and sum the layers
    rgb_values_reshaped = opacity * colours_batch.unsqueeze(-1).unsqueeze(-1)
    final_image_layers = rgb_values_reshaped * kernel_rgb_cropped
    partial_image = final_image_layers.sum(dim=0)
    
    return partial_image

def generate_dead_leaves_images(rmin, rmax, alpha, opacity, color_histogram, num_gaussians,gaussian_batch_size=2000, kernel_size=1001, device='cpu', image_size=(500,500,3)):
    ...
    for start in range(0, num_gaussians, gaussian_batch_size):
        ...
        partial_image = checkpoint(
            ...
            xy_base, device, start, end, image_width = image_size[0], image_height = image_size[1], kernel_size=kernel_size,
...
def train():
    ...
    gaussian_increment = 50
    gaussian_increment_freq = 30
    ...
    for epoch in range(1,num_epochs+1):
        ...
        if epoch % gaussian_increment_freq == 0:
            num_gaussians = num_gaussians + gaussian_increment
        gc.collect()
